...
export default function CartPage() {
  ...
  const validateCart = async () => {
    for (const item of cartItems) {
      const { data: product, error } = await supabase.from("products").select("stock, name").eq("id", item.id).single();
      if (error) {
        toast.error("Error checking stock. Please try again.");
        return false;
      }
      if (!product || product.stock < item.quantity) {
        toast.error(`"${item.name}" exceeds available stock. Only ${product?.stock ?? 0} left.`);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (cartItems.some((c) => c.quantity === 0)) {
        toast.error("Each item must have quantity greater than 0");
        setLoading(false);
        return;
      }
  ...
  const handleIncrease = async (item: CartItem) => {
    ...
    if (error) {
      ...
      toast.error("Unable to check stock right now.");
      return;
    }
    if (!product || item.quantity >= product.stock) {
      toast.error(`Only ${product?.stock ?? 0} in stock right now.`);
      return;
    }
      ...
      {cartItems.length === 0 ? 
        ...
        <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <button className="px-2 py-1 bg-blue-500 rounded hover:bg-blue-600 flex items-center justify-center w-8 h-8"
                        onClick={() => handleDecrease(item)}>
                        –
                      </button>
                      <span>{item.quantity}</span>
                      <SpinnerButton onClick={() => handleIncrease(item)} loading={loadingIds.includes(item.id)} disabled={item.quantity >= item.maxStock}>+</SpinnerButton>
                    </div>
                    <p className="font-semibold mt-1">Total: ₦{item.price * item.quantity}</p>
                  </div>
                </div>

                <button onClick={() => removeFromCart(item.id)} className="flex gap-2 items-center text-red-500 cursor-pointer">  Remove <FaTrash /> </button>
              </li>
            ))}
          </ul>
          <div className="bg-white p-4 rounded-lg shadow flex flex-col md:flex-row justify-between items-center mt-6">
            ...
            <button onClick={handleCheckout} className="mt-3 md:mt-0 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
              Proceed to Checkout
            </button>
          </div>
        </>
